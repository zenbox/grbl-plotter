...
function sethandlers() {
  var c;
  //! New handler
  $("#opengcode2arduinodialog").on("click", opengcode2arduinodialog);
  ...
  $("#rmode").on("click", function () {
    setcncmode(0);
  });
  $("#lmode").on("click", function () {
    setcncmode(1);
  });
  $("#origin0").on("click", function () {
    setcncorigin(0);
  });
  $("#origin1").on("click", function () {
    setcncorigin(1);
  });
  $("#origin2").on("click", function () {
    setcncorigin(2);
  });
  $("#origin3").on("click", function () {
    setcncorigin(3);
  });
  $("#origin4").on("click", function () {
    setcncorigin(4);
  });
  $("#svgfile").on("change", svgfilechanged);
  $("#importbutton").on("click", dofileselected);
  $("#ncwidth").on("input", function () {
    dialogwidthchanged("ncwidth", "ncheight");
  });
  $("#ncheight").on("input", function () {
    dialogheightchanged("ncwidth", "ncheight");
  });
  $("#generategcode").on("click", generategcode);

  // ! ********
  $("#sendgcode").on("click", sendgcode);

  $("#svgncwidth").on("input", function () {
    dialogwidthchanged("svgncwidth", "svgncheight");
  });
  $("#svgncheight").on("input", function () {
    dialogheightchanged("svgncwidth", "svgncheight");
  });
  $("#scadncwidth").on("input", function () {
    dialogwidthchanged("scadncwidth", "scadncheight");
  });
  $("#scadncheight").on("input", function () {
    dialogheightchanged("scadncwidth", "scadncheight");
  });
...
/**
 * @desc   New save to arduino dialog
 * @since  2023-08-13
 * @author Michael
 * ! ************
 */
function opengcode2arduinodialog() {
  setsvgtexts();
  $("#grblmode option")[grblmode].selected = true;
  $("#safez").val(safez);
  $("#movespeed").val(g0feed);
  ...
  $("#gcodeerror").hide();
  //! ******
  $("#sendgcodefiledialog").modal("show");
}
...
function generatescadexport() {
  ...
  if (w > 0 && h > 0 && !isNaN(w) && !isNaN(h)) validated = true;
  if (validated === false) {
    $("#svgexporterror").show();
    return;
  }
...
function makethemesdropdown() {
  ...
  for (c = 0; c < themes.length; c++) {
    ...
    txt +=
      '<a id="' +
      id +
      '"class="dropdown-item navbardropdownitems" href="#" data-toggle="modal" >' +
      tname +
      "</a>";
  }
  $("#themesdropdownitems").html(txt);
  $("#thm0").on("click", function () {
    changetheme(0);
  });
  $("#thm1").on("click", function () {
    changetheme(1);
  });
  $("#thm2").on("click", function () {
    changetheme(2);
  });
  $("#thm3").on("click", function () {
    changetheme(3);
  });
  $("#thm4").on("click", function () {
    changetheme(4);
  });
  $("#thm5").on("click", function () {
    changetheme(5);
  });
}
...
function makesamplesdropdown() {
  ...
  $("#sam0").on("click", function () {
    opensamplesdialog(0);
  });
  $("#sam1").on("click", function () {
    opensamplesdialog(1);
  });
  $("#sam2").on("click", function () {
    opensamplesdialog(2);
  });
  $("#sam3").on("click", function () {
    opensamplesdialog(3);
  });
  $("#sam4").on("click", function () {
    opensamplesdialog(4);
  });
  $("#sam5").on("click", function () {
    opensamplesdialog(5);
  });
  $("#sam6").on("click", function () {
    opensamplesdialog(6);
  });
}
...
function setsampledescription(sn) {
  ...
  if (stype === 0)
    txt += '<p class="sampledesctext"><b>Project type:</b> Router</p>';
  if (stype === 1)
    txt += '<p class="sampledesctext"><b>Project type:</b> LASER</p>';
  if (stype === 2)
    txt += '<p class="sampledesctext"><b>Project type:</b> Router or LASER</p>';
  if (slevel === 0)
    txt += '<p class="sampledesctext"><b>Skill level:</b> Beginner</p>';
  if (slevel === 1)
    txt += '<p class="sampledesctext"><b>Skill level:</b> Intermediate</p>';
  if (slevel === 2)
    txt += '<p class="sampledesctext"><b>Skill level:</b> Advanced</p>';
  txt +=
    '<p class="sampledesctext"><b>Dimensions (WxH):</b> ' +
    swidth +
    "mm x " +
    sheight +
    "mm</p>";
  ...
  if (surl !== "") {
    ...
    txt +=
      "Click here to visit the web page for this project (link opens in a new tab).";
    txt += "</a></p>";
  }
  txt += "<p>" + ssvg + "</p>";
